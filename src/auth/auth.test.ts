...
describe('webLogin', () => {
  ...
  it('should not open browser when isOpenBrowser is false', async () => {
    mockAxios.post.mockResolvedValue({
      data: { result: { apiKey: 'new-api-key' } },
    });
    mockFsExtra.ensureDir.mockResolvedValue(undefined);
    ...
    await webLogin(false);

    expect(mockOpenBrowser).not.toHaveBeenCalled();
    expect(mockSpinner.succeed).toHaveBeenCalled();
  });

  it('should not open browser when isOpenBrowser is undefined (default)', async () => {
    mockAxios.post.mockResolvedValue({
      data: { result: { apiKey: 'new-api-key' } },
    });
    mockFsExtra.ensureDir.mockResolvedValue(undefined);
